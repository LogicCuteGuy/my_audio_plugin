...
impl Default for NoteTables {
    fn default() -> Self {
        Self {
            i2t: AtomicCell::new(NoteTablesArray { i84: [0; 84] }),
            im2t: AtomicCell::new(NoteTablesArray { i84: [0; 84] })
        }
    }
}
...
pub mod serialize {
    ...
    pub fn serialize<S, T>(cell: &[T; 84], serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
            T: Serialize + Copy,
    {
    ...
    pub fn deserialize<'de, D, T>(deserializer: D) -> Result<[T; 84], D::Error>
        where
            D: Deserializer<'de>,
            T: Deserialize<'de> + Copy,
    {
        let result: Result<T, _> = T::deserialize(deserializer);
        result.map(|val| [val; 84])
    }
}