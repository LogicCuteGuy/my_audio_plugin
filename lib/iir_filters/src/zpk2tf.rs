...
pub struct BACoeffs {
    pub(crate) b: Vec<Complex<f32>>,
    pub(crate) a: Vec<Complex<f32>>,
}

pub(crate) fn zpk2tf(z: &[Complex<f32>], p:&[Complex<f32>], k: f32) -> Result<BACoeffs, Error> {
    if z.is_empty() && p.is_empty() {
        return Ok( BACoeffs{ b: vec_cplx![(1.0, 0.0)], a: vec_cplx![(1.0, 0.0)] } );

    }

    let mut b: Vec<Complex<f32>> = poly(z)?.iter().map(|x| k*x ).collect();
    let mut a = poly(p)?;
    ...
    if all_conjugates(&b)? {
        b = b.iter()
            .map(|x| Complex::<f32>::new(x.re, 0.0))
...
fn all_conjugates(roots: &[Complex<f32>]) -> Result<bool, Error> {

    let mut pos_roots:Vec<Complex<f32>> = roots
        .iter()
        .copied()
        .filter(|x| x.im > 0.0)
        .collect();

    let mut neg_roots:Vec<Complex<f32>> = roots
        .iter()
        .filter(|x| x.im < 0.0)
        .map(|x| x.conj())
        .collect();
...
mod tests {
    ...
    #[test]
    fn test_identity() {
        let z:Vec<Complex<f32>> = vec![];
        let p:Vec<Complex<f32>> = vec![];
        let k = 1.0;
    ...
    #[test]
    fn test_1() {
        let z:Vec<Complex<f32>> = vec_cplx![(-1.0,0.0), (-1.0,0.0)];
        let p:Vec<Complex<f32>> = vec_cplx![(0.57149, 0.2936), (0.57149, -0.2936)];
        let k = 1.0;
