...
type FnConsumeCplx = fn(&mut HashMap<usize, Complex<f32>>, pred: &dyn Fn(Complex<f32>) -> bool) -> Result<Complex<f32>, Error>;
...
impl SosCoeffs {
    pub(crate) fn from_array(arr: &[f32;6]) -> Self {
        SosCoeffs {
            b0: arr[0],
            b1: arr[1],
            b2: arr[2],
...
impl Sos {
    ...
    pub fn from_vec(vec: Vec<[f32; 6]>) -> Self {
...
pub fn zpk2sos(zpk: &ZPKCoeffs, pairing: Option<SosPairing>) -> Result<Sos, Error> {
    ...
    match pairing {
        ...
        _ => {
            ...
            p.append( &mut vec![Complex::<f32>::zero(); max(z.len() as i32 - p.len() as i32, 0) as usize]);
            z.append( &mut vec![Complex::<f32>::zero(); max(p.len() as i32 - z.len() as i32, 0) as usize]);

            n_sections = (max(p.len(), z.len()) + 1) / 2;

            if p.len() % 2 == 1 && pairing == Nearest {
                p.push(Complex::<f32>::zero());
                z.push(Complex::<f32>::zero());
            }
    ...
    let mut p: HashMap<usize, Complex<f32>> = p.iter().enumerate().map(|(i,c)| (i, *c)).collect::<HashMap<_, _>>();
    let mut z: HashMap<usize, Complex<f32>> = z.iter().enumerate().map(|(i,c)| (i, *c)).collect::<HashMap<_, _>>();

    //Construct the system, reversing order so the "worst" are last
    let mut sos = vec![vec![Complex::<f32>::zero(); 6]; n_sections];
        ...
        if is_real(p1) && num_remaining_reals == 0 {
            ...
            if pairing != Minimal {
               ...
               // sos[si] = _single_zpksos([z1, 0], [p1, 0], 1)
                for (i, e) in single_zpksos(&[z1, Complex::<f32>::zero()], &[p1, Complex::<f32>::zero()], 1.0)?.iter().enumerate() {
                    sos[si][i] = *e;
                }
            } else if !z.is_empty() {
                let z1 = find_and_remove_closest(&mut z, NumberType::Real, &p1)?;
...
fn single_zpksos(z: &[Complex<f32>], p:&[Complex<f32>], k: f32) -> Result<Vec<Complex<f32>>, Error> {
    let mut sos = vec![Complex::<f32>::zero(); 6];
    let ba = zpk2tf(z, p, k)?;
...
fn find_and_remove_closest_to_unit_circle(roots: &mut HashMap<usize, Complex<f32>>,  pred: &dyn Fn(Complex<f32>) -> bool) -> Result< Complex<f32>, Error> {
    if roots.is_empty() {
...
fn find_and_remove_closest(roots: &mut HashMap<usize, Complex<f32>>, kind: NumberType, to: &Complex<f32> ) -> Result< Complex<f32>, Error> {
    if roots.is_empty() {
        return Err(InternalError("roots list should not be empty.".to_string()));
    }
...
mod tests {
    ...
    let deg2rad = PI / 180.0;
        ...
        z.append(&mut z.iter().map(|x| x.conj()).collect::<Vec<Complex<f32>>>());

        let mut p = thetas.iter().zip(mags).map(|(theta, mag)| mag * (theta * deg2rad * i).exp() ).collect::<Vec<Complex<f32>>>();
        p.append(&mut p.iter().map(|x| x.conj()).collect::<Vec<Complex<f32>>>());
    ...
    fn test_jackson_2(#[case] pairing:Option<SosPairing>) {
        ...
        let mut p = thetas_p.iter().zip(mags).map(|(theta, mag)| mag * (theta * deg2rad * i).exp() ).collect::<Vec<Complex<f32>>>();
        p.append(&mut p.iter().map(|x| x.conj()).collect::<Vec<Complex<f32>>>());
