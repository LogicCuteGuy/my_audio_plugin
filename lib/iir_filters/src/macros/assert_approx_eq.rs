 ...
 right: `{:?}`
   eps: `{:?}`: {}"#, &*left_val, &*right_val, &*epsilon,
    ...
    mod tests {
        #[test]
        fn compare_with_eps() {
            assert_approx_eq!(3f32, 4f32, 2f32);
        }

        #[test]
        #[should_panic]
        fn bad_compare_with_eps() {
            assert_approx_eq!(3f32, 4f32, 1e-3f32);
        }
 ...
 right: `4.0`
   eps: `0.001`: Temperature value incorrect.")]
        fn check_error_message() {
            assert_approx_eq!(3f32, 4f32, 1e-3f32, "Temperature value incorrect.");
        }
        ...
        fn should_evaluate_eps_only_once() {
            ...
            assert_approx_eq!(0_f32, 100_f32, {
            count += 1_f32;
            count
        });
    }
